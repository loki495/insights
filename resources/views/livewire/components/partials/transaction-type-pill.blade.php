...
<template x-if="!optimisticTypes[{{ $transaction['id'] }}]">
    @if($style)
    <button type="button" @click="$dispatch('edit-type', { transaction_id: {{ $transaction['id'] }} })" class="cursor-pointer text-xs px-1.5 py-0.5 rounded-md text-nowrap text-white [text-shadow:0_1px_2px_rgb(0_0_0_/_70%)] {{ $style['class'] }}">{{ $style['label'] }}</button>
    @else
    <button type="button" @click="$dispatch('edit-type', { transaction_id: {{ $transaction['id'] }} })" class="cursor-pointer text-xs px-1.5 py-0.5 rounded-md text-nowrap border border-dashed border-zinc-400 text-zinc-500 dark:text-zinc-400">Unclassified</button>
    @endif
</template>
