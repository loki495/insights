...
Everything in [Getting Started](#getting-started-local-development) above is a **development**
setup — hot-reloading Vite, debug output on, `docker-compose.yml`'s image built from
`docker/setup-dev-container.sh` (SSH server, Node/Playwright for browser tests, `opcache`
disabled). None of that belongs in a real deployment. Use this instead.
